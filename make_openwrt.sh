...
# ENV
OPENWRT_PATH="/home/eallion/Openwrt"
...
# 修改路由器 IP 地址
sed -i "s/192.168.1.1/$LAN_IP/g" "$OPENWRT_PATH/package/base-files/files/bin/config_generate"
...
# 进入源目录
cd "$OPENWRT_PATH/bin/targets/x86/64/" || { echo "Source directory not found"; exit 1; }
    ...
    if [ -n "$files_to_copy" ]; then
        ...
        for file in $files_to_copy; do
            ...
            if [ $? -eq 0 ]; then
                ...
                sha256_value=$(grep --color=none -E "\*$filename" "$OPENWRT_PATH/bin/targets/x86/64/sha256sums" | awk '{print $1}')
...
echo "Sha256 Hash:"
grep --color=none -E "($(IFS='|'; echo "${FILE_TYPES[*]}"))" "$OPENWRT_PATH/bin/targets/x86/64/sha256sums"
echo "Openwrt build successful!"